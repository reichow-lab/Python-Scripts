    ...
    for i in range(0,num_bins,1):
        for j in range(0,num_bins,1):
            for c in range(0,num_bins,1):
                counter = counter + pop_matrix[c,i]
            if counter == 0:
                pass
            else:
                markov[j,i] = pop_matrix[j,i]/counter
                counter = 0
    return markov
# rate2gibbs is out of date since the creation of mfpt(), which calculates the PMF from stationary probabilies
# as opposed to calculating the PMF from relative transition probabilities.
def rate2gibbs(num_bins, edge, tran_matrix, bin_size, outname):
    sum     = 0
    ...
    def det_bal(forw, rev):
        ...
        return gibb
    for j in range(1,num_bins,1):    # This loop is the first to populate the matrix[i,0] column. I need to incrememnt edge by bin_size instead of 1.
