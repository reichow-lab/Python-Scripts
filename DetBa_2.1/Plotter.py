...
if args.Wchoice == True:
    ...
    if args.Wchoice == True and args.Tchoice == True:
        ...
        plt.xlabel("Time (ns)")
        ax = sns.lineplot(data=FinalDF,x="Time (ns)",y="Water Flux (ns^-1)",ax=ax,color="#00A6ED",label='Water',legend=False,linewidth=0)
        ax2 = sns.lineplot(data=FinalDF,x="Time (ns)",y="Ionic Current (pA)",ax=ax2,color="#F6511D",label='K+',legend=False,linewidth=0)
        fig.legend()
