...
def WatFluxTrack(system,outname,palette,WS,LT,d_col,watlim):
    ...
    for FILE, label in zip(FileList, labels):
        ...
        sns.lineplot(data=CumPermeations, x="Time (ns)", y="Cumulative Permeations", hue="Pore Height", palette=sns.color_palette(palette, n_colors=5))
        ...
        sns.lineplot(data=CumAverage, x="Time (ns)", y="Cumulative Average", hue="Pore Height", palette=sns.color_palette(palette, n_colors=5))
