...
def WatFluxTrack(system,outname,palette,WS,LT,d_col):
    ...
    for FILE, label in zip(FileList, labels):
        ...
        sns.lineplot(data=CumPermeations, x="Time (ns)", y="Cumulative Water Permeations", hue=Final[2], palette=sns.color_palette(palette, n_colors=len(FileList)))
        plt.savefig(outname+"_CumWaterPerm.png", dpi=400)
        plt.clf()
        fig, ax = plt.subplots()
        plt.xlabel("Time (ns)")
        plt.ylabel("Cumulative Avg. Water Flux (ns^-1)")
        sns.lineplot(data=CumAverage, x="Time (ns)", y="<current> (pA)", hue=Final[2], palette=sns.color_palette(palette, n_colors=len(FileList)))
