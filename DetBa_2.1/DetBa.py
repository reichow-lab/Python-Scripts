...
while END == False:
	...
	if choice == 'R':
		...
		d_col		=	int(raw_input("Which column from your data_file will you use? "))

		array_dim	=	1

		init_matrix	=	initialize(init, fina, bin_size, outname, array_dim)

		pop_matrix	=	populate(init_matrix, prefix, num_files, fina, bin_size, num_bins, array_dim, d_col)

		pop_matrix.dump(out_pop_mat)

		rate_matrix	=	pop2rate(init, fina, pop_matrix, bin_size)

		rate_matrix.dump(out_rate_mat)

		gibbs		=	rate2gibbs(init, fina, rate_matrix, bin_size, outname)
		...
		write_mat	=	hist_write(init, pop_matrix, outname, bin_size, num_bins)
	...
	choice		=	str(raw_input("What would you like to do? Rates PMF (R), Histogram (H), Ion Tracker (T), I-V approximator (I), Exit (E) "))
	choice		=	choice.upper()
